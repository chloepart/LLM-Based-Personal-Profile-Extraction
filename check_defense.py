...
from LLMPersonalInfoExtraction.utils import open_config, open_txt, parsed_data_to_string

task_config = open_config('./configs/task_configs/synthetic.json')
task_manager, _ = PIE.create_task(task_config)
defense = PIE.create_defense('replace_at')

raw_list, curr_label = task_manager[0]
...
processed = defense.apply(parsed_data_to_string('synthetic', processed_data, 'llama-3.1-8b-instant'), curr_label)
...
for line in processed.split('\n'):
    if 'mail' in line.lower() or 'AT' in line or '@' in line:
        print(repr(line))
from LLMPersonalInfoExtraction.utils import load_instruction, open_txt
